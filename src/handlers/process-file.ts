...
export async function handler(event: SQSEvent): Promise<void> {
  for (const sqsRecord of event.Records) {
    ...
    try {
      ...
      logError('Invalid SQS message body (expected S3 event JSON)', { messageId: sqsRecord.messageId });
      throw new Error('Invalid SQS message body');
    }
    if (!s3Event.Records?.length) {
      logInfo('SQS message has no S3 records, skipping', { messageId: sqsRecord.messageId });
      continue;
    }
    for (const record of s3Event.Records) {
      ...
      try {
        ...
        // Always rethrow so the message is retried and eventually sent to the DLQ
